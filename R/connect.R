...
csv_s3 <- function(route=0){
    if (route==0) {
the_file <- aws.s3::s3read_using(read.csv, object = "s3://pub-raw/diccionarios/catalogo_beneficio.csv",stringsAsFactors=FALSE)
return(the_file)
    }
    else{
    the_file <- aws.s3::s3read_using(read.csv, object = route,stringsAsFactors=FALSE)
    return(the_file)
    }
}
